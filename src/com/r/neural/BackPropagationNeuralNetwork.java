...
public class BackPropagationNeuralNetwork {
	...
	protected ActivationFunction[] activationFunction = new ActivationFunction[] { new TanhActivationFunction() };
	...
	public double getLearningRate(int layer) {
		if (layer < learningRate.length)
			return learningRate[layer];
	...
	public ActivationFunction getActivationFunction(int layer) {
		if (layer < activationFunction.length)
			return activationFunction[layer];
		return activationFunction[0];
	}

	public double[] getInputs() {
		return neuronValue[0];
	}

	public double[] getOutputs() {
		return neuronValue[numLayers - 1];
	}
	...
	public void forwardPropagate() {
		for (int layer = 1, prevLayer = 0; layer < numLayers; layer++, prevLayer++) {
			...
			for (int neuron = 0; neuron < numNeurons[layer]; neuron++) {
				double sum = 0.0;
				for (int prevNeuron = 0; prevNeuron < (numNeurons[prevLayer]); prevNeuron++) {
					sum += neuronValue[prevLayer][prevNeuron]
							* weight[prevLayer][prevNeuron][neuron];
				}
				neuronValue[layer][neuron] = act.calc(sum
						- threshold[layer][neuron]);
			}
		}
	}
	...
	public void backPropagate(double[] expectedOutputs) {
		...
		for (int layer = numLayers - 1, prevLayer = numLayers - 2, nextLayer = numLayers; layer > 0; layer--, prevLayer--, nextLayer--) {
			double prevLearningRate = getLearningRate(prevLayer);
			ActivationFunction act = getActivationFunction(layer);
			for (int neuron = 0; neuron < numNeurons[layer]; neuron++) {
				if (layer == outputLayer) {
					errorGradient[layer][neuron] = act
							.calcInverse(neuronValue[layer][neuron])
							* (expectedOutputs[neuron] - neuronValue[layer][neuron]);
				} else {
					double sum = 0.0;
					for (int nextNeuron = 0; nextNeuron < numNeurons[nextLayer]; nextNeuron++) {
						sum += errorGradient[nextLayer][nextNeuron]
								* weight[layer][neuron][nextNeuron];
					}
					errorGradient[layer][neuron] = act
							.calcInverse(neuronValue[layer][neuron]) * sum;
				}
				for (int prevNeuron = 0; prevNeuron < numNeurons[prevLayer]; prevNeuron++) {
					double weightChange = prevLearningRate
							* neuronValue[prevLayer][prevNeuron]
							* errorGradient[layer][neuron];
					weight[prevLayer][prevNeuron][neuron] += weightChange
							+ (lastWeightChange[prevLayer][prevNeuron][neuron] * momentum);
					lastWeightChange[prevLayer][prevNeuron][neuron] = weightChange;
				}
				threshold[layer][neuron] -= prevLearningRate
						* errorGradient[layer][neuron];
			}
		}
	}
	...
	public String encode() {
		...
		for (int layer = 0, nextLayer = 1; nextLayer < numLayers; layer++, nextLayer++) {
			for (int neuron = 0; neuron < neuronValue[layer].length; neuron++) {
				for (int nextNeuron = 0; nextNeuron < neuronValue[nextLayer].length; nextNeuron++) {
					sb.append("weight[" + layer + "][" + neuron + "]["
							+ nextNeuron + "]:");
					sb.append(Double
							.toString(weight[layer][neuron][nextNeuron]));
					sb.append(nl);
				}
			}
		}
		...
		for (int layer = 0, nextLayer = 1; nextLayer < numLayers; layer++, nextLayer++) {
			for (int neuron = 0; neuron < neuronValue[layer].length; neuron++) {
				for (int nextNeuron = 0; nextNeuron < neuronValue[nextLayer].length; nextNeuron++) {
					sb.append("lastWeightChange[" + layer + "][" + neuron
							+ "][" + nextNeuron + "]:");
					sb.append(Double
							.toString(lastWeightChange[layer][neuron][nextNeuron]));
					sb.append(nl);
				}
			}
		}
