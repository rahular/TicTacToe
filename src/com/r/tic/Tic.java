...
public class Tic {
	...
	public void playGame(PrintStream out, boolean outputGridAtEachStep,
			boolean outputWinnerAtEnd) {
		init();
	...
	public boolean nextMove(char whichPlayer) {
		...
		if ((whichPlayer != 'X') && (whichPlayer != 'O')) {
			...
			if ((whichPlayer != 'X') && (whichPlayer != 'O')) {
				throw new Error(
						"nextMove(): whichPlayer must be either 'X' or 'O'");
			}
		}
	...
	public char[][] getGridBeforeMove(int moveIdx) {
		return gridBeforeMove[moveIdx];
	}

	public int getMoveRow(int moveIdx) {
		return moveRow[moveIdx];
	}

	public int getMoveCol(int moveIdx) {
		return moveCol[moveIdx];
	}
