...
public class GeneticPlayer2 extends Player {
	...
	// /
	/*
	 * private static boolean playerHasDoubleWin(char[][] grid, char
	 * whichPlayer) { int r, c, rc, pc, sc, src; boolean[][] isWinSpace = new
	 * boolean[3][3];
	 * 
	 * for (r = 0; r < 3; r++) { pc = sc = 0; src = -1; for (c = 0; c < 3; c++)
	 * { if (grid[r][c] == whichPlayer) { pc++; } else if (grid[r][c] == ' ') {
	 * sc++; if (src < 0) src = c; } } if ( (pc == 2) && (sc == 1) ) {
	 * isWinSpace[r][src] = true; } }
	 * 
	 * for (c = 0; c < 3; c++) { pc = sc = 0; src = -1; for (r = 0; r < 3; r++)
	 * { if (grid[r][c] == whichPlayer) { pc++; } else if (grid[r][c] == ' ') {
	 * sc++; if (src < 0) src = r; } } if ( (pc == 2) && (sc == 1) ) {
	 * isWinSpace[src][c] = true; } }
	 * 
	 * pc = sc = 0; src = -1; for (rc = 0; rc < 3; rc++) { if (grid[rc][rc] ==
	 * whichPlayer) { pc++; } else if (grid[rc][rc] == ' ') { sc++; if (src < 0)
	 * src = rc; } } if ( (pc == 2) && (sc == 1) ) { isWinSpace[src][src] =
	 * true; }
	 * 
	 * pc = sc = 0; src = -1; for (rc = 0; rc < 3; rc++) { if (grid[rc][2-rc] ==
	 * whichPlayer) { pc++; } else if (grid[rc][2-rc] == ' ') { sc++; if (src <
	 * 0) src = rc; } } if ( (pc == 2) && (sc == 1) ) { isWinSpace[src][2-src] =
	 * true; }
	 * 
	 * int numWinSpaces = 0; for (r = 0; r < 3; r++) { for (c = 0; c < 3; c++) {
	...
	public void train(int freeCount) {
		if (allBoards == null) {
			...
			for (int boardIdx = 0; boardIdx < allBoards.length; boardIdx++) {
				...
				// playerHasDoubleWin(allBoards[boardIdx], 'X');
				// / allBoardsOHasDoubleWin[boardIdx] =
				// playerHasDoubleWin(allBoards[boardIdx], 'O');
		...
		int numGames = 1;
		for (int fc = freeCount - 1; fc > 0; fc -= 2)
			numGames *= fc;
		...
		while (true) {
			System.out.println(" learning freeCount=" + freeCount
					+ " noImprovementPasses=" + noImprovementsPasses);
				...
				if ((9 - (allBoardsXMoves[boardIdx] + allBoardsOMoves[boardIdx])) == freeCount) {
					if (gax[boardIdx] != null) {
						...
						if (oldFitness < numGames) {
							...
							// No need to recalculate fitness for survivors
